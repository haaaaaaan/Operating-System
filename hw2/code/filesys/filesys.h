...
//  The OpenAFile function is used for kernel open system call
    OpenFileId OpenAFile(char *name) {
        ...
        if(OpenFile_num>=20) return -1;
        for(int idx=0;idx<20;idx++)
        {
            if(OpenFileTable[idx]==NULL)
            {
                OpenFile* opened_file = new OpenFile(fileDescriptor);
                OpenFileTable[idx]= opened_file;
                //printf("openAFile %d\n",idx);
                OpenFile_num++;
                return idx;
            } 
    ...
    int WriteFile(char *buffer, int size, OpenFileId id){
        int flag = 0;
        if(id>=20 || id < 0) return -1;
        //printf("writeFile %d\n",id);
        OpenFile* file = OpenFileTable[id];
        if(file) flag=1;
        if(flag == 1) return file->Write(buffer, size);
        else return -1;
    }
    int ReadFile(char *buffer, int size, OpenFileId id){
        ...
        OpenFile* file = OpenFileTable[id];
        if(file) flag=1;
        if(flag == 1) return file->Read(buffer, size);
        else return -1;
    }
